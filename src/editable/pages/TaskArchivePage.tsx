  ...
  return (
    <EditableSiteShell>
      <main className="min-h-screen bg-[#111010] text-white">
        <section className="border-b border-white/10 bg-[#111010]">
          <div className="mx-auto flex max-w-[1280px] flex-col gap-6 px-4 py-16 text-center sm:px-6 lg:px-8 lg:py-24">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.28em] text-[var(--slot4-accent)]">Distribution library</p>
              <h1 className="mcp-gradient-text mt-4 text-5xl font-medium leading-tight tracking-[-0.045em] sm:text-7xl">
                {category === 'all' ? label : categoryLabel}
              </h1>
            </div>
            <p className="mx-auto max-w-2xl text-base font-semibold leading-8 text-white/70">
              Timely releases, company updates, and public announcements organized for fast discovery across the media network.
            </p>
          </div>
        </section>

        <section className="border-b border-white/10 bg-[#1b1818] text-white">
        ...
        {lead ? (
          <section className="mx-auto grid max-w-[1280px] gap-5 px-4 py-10 sm:px-6 lg:grid-cols-[1.6fr_.75fr] lg:px-8">
            <Link href={`${basePath}/${lead.slug}`} className="group relative min-h-[34rem] overflow-hidden rounded-lg mcp-glow-card">
              <img src={getImage(lead)} alt="" className="absolute inset-0 h-full w-full object-cover opacity-85 transition duration-700 group-hover:scale-[1.025]" />
              <div className="absolute inset-0 bg-gradient-to-t from-[#111010] via-black/15 to-transparent" />
              <div className="absolute inset-x-0 bottom-0 p-6 text-white sm:p-9">
                <span className="rounded bg-[var(--slot4-accent)] px-3 py-2 text-[10px] font-black uppercase tracking-[0.2em] text-black">{getCategory(lead, label)}</span>
                <h2 className="mt-5 max-w-4xl text-4xl font-medium leading-[1.03] tracking-[-0.045em] sm:text-6xl">{lead.title}</h2>
                <p className="mt-5 max-w-2xl line-clamp-2 text-sm font-semibold leading-7 text-white/80">{getSummary(lead)}</p>
              </div>
            </Link>
            <div className="grid">
              <div className="rounded-t-lg border border-white/12 bg-[#252221] p-6 text-white">
                <p className="text-xs font-black uppercase tracking-[0.24em]">Top stories</p>
                <p className="mt-3 text-3xl font-medium leading-tight">What the distribution desk is watching now.</p>
              </div>
              {secondary.map((post, index) => (
                <Link key={post.id || post.slug} href={`${basePath}/${post.slug}`} className="group grid grid-cols-[7rem_1fr] border-x border-b border-white/12 bg-[#1b1918] last:rounded-b-lg">
                  <img src={getImage(post)} alt="" className="h-full min-h-40 w-full object-cover opacity-75 transition group-hover:opacity-100" />
                  <div className="p-5">
        ...
        <section className="mx-auto max-w-[1280px] px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
          <div className="mb-8 flex flex-wrap items-end justify-between gap-5 border-b border-white/15 pb-5">
            <h2 className="text-4xl font-medium tracking-[-0.04em] sm:text-5xl">More from the desk</h2>
            <form action={basePath} className="flex overflow-hidden rounded border border-white/20 bg-white text-black">
              ...
              <button className="h-11 bg-[var(--slot4-accent)] px-5 text-xs font-black uppercase tracking-[0.14em] text-black">Filter</button>
            </form>
          </div>
          ...
          {remaining.length ? (
            <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
              {remaining.map((post, index) => (
                <Link key={post.id || post.slug} href={`${basePath}/${post.slug}`} className="group overflow-hidden rounded-lg border border-white/12 bg-[#1b1918] transition hover:-translate-y-1 hover:shadow-[0_20px_70px_rgba(119,238,229,.12)]">
                  ...
                  <div className="p-5">
                    ...
                    <h3 className="mt-4 text-2xl font-semibold leading-[1.08] tracking-[-.035em]">{post.title}</h3>
            ...
            <div className="rounded-lg border border-dashed border-white/20 bg-[#1b1918] p-12 text-center">
              <Search className="mx-auto h-8 w-8" />
              <h2 className="mt-4 text-3xl font-medium">No stories found</h2>
              <p className="mt-2 text-sm text-white/60">Try another category or publish a new media update.</p>
            </div>
          ) : null}

          <div className="mt-10 flex items-center justify-center gap-0">
            {pagination.hasPrevPage ? <Link href={pageHref(basePath, category, page - 1)} className="rounded-l border border-white/20 bg-white px-5 py-3 text-xs font-black uppercase text-black">Previous</Link> : null}
            <span className="border-y border-white/20 bg-[var(--slot4-accent)] px-5 py-3 text-xs font-black uppercase text-black">Page {page} / {pagination.totalPages || 1}</span>
            {pagination.hasNextPage ? <Link href={pageHref(basePath, category, page + 1)} className="rounded-r border border-white/20 bg-white px-5 py-3 text-xs font-black uppercase text-black">Next</Link> : null}
          </div>
        </section>
      </main>
    </EditableSiteShell>
