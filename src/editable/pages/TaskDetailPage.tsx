  ...
  return (
    <section className="bg-[#111010] text-white">
      <header className="border-b border-white/10">
        <div className="mx-auto max-w-[1180px] px-4 py-12 text-center sm:px-6 lg:px-8 lg:py-20">
          <BackLink task={task} />
          <div className="mx-auto mt-10 flex max-w-3xl flex-wrap items-center justify-center gap-5 border-t border-white/15 pt-5 text-[11px] font-black uppercase tracking-[0.16em] text-white/62">
            <span className="text-[var(--slot4-accent)]">{categoryOf(post, 'News')}</span>
            {published ? <time>{published}</time> : null}
          </div>
          <h1 className="mcp-gradient-text mx-auto mt-6 max-w-6xl text-5xl font-medium leading-[1.05] tracking-[-0.05em] sm:text-6xl lg:text-[5rem]">{post.title}</h1>
          {summaryText(post) ? <p className="mx-auto mt-6 max-w-4xl text-xl font-semibold leading-8 text-white/70 sm:text-2xl">{summaryText(post)}</p> : null}
        </div>
      </header>

      {images[0] ? (
        <figure className="mx-auto max-w-[1320px] px-4 pt-10 sm:px-6 lg:px-8">
          <div className="overflow-hidden rounded-lg mcp-glow-card">
            <img src={images[0]} alt="" className="max-h-[760px] w-full object-cover" />
            <figcaption className="border-t border-white/10 bg-[#1b1918] px-4 py-3 text-xs text-white/55 sm:px-6">Featured image for {post.title}</figcaption>
          </div>
        </figure>
      ) : null}

      <div className="mx-auto grid max-w-[1180px] gap-12 px-4 py-12 sm:px-6 lg:grid-cols-[minmax(0,760px)_320px] lg:px-8 lg:py-16">
        <article className="min-w-0 rounded-lg bg-white p-6 text-[#111010] shadow-[0_24px_80px_rgba(0,0,0,.22)] sm:p-9">
          <BodyContent post={post} />
          <EditableComments slug={post.slug} comments={comments} />
        </article>
        <div className="rounded-lg border border-white/12 bg-[#1b1918] p-5">
...
function RelatedPanel({ task, post, related, compact = false }: { task: TaskKey; post: SitePost; related: SitePost[]; compact?: boolean }) {
  ...
  return (
    <aside className="min-w-0 space-y-5">
      {!compact ? (
        <div className="rounded-lg border border-white/12 bg-[#252221] p-5 text-white">
...
function RelatedCard({ task, post }: { task: TaskKey; post: SitePost }) {
  ...
  return (
    <Link href={buildPostUrl(task, post.slug)} className="group flex gap-3 border-t border-white/12 py-3 transition hover:text-[var(--slot4-accent)]">
      {image && task !== 'sbm' ? <img src={image} alt="" className="h-20 w-20 shrink-0 rounded object-cover" /> : <div className="flex h-20 w-20 shrink-0 items-center justify-center rounded bg-black text-white"><FileText className="h-6 w-6" /></div>}
...
function EditableComments({ slug, comments }: { slug: string; comments: Array<{ id: string; name: string; comment: string; createdAt: string }> }) {
  return (
    <section className="mt-12 rounded-lg border border-black/10 bg-[#f4f6f8] p-5">
